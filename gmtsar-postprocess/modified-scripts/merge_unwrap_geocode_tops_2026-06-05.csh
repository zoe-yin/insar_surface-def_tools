...
#    H. Zoe Yin, June 2026: modified to merge xphase and yphase phasegradient grids 
#
...
#
  ...
  if (-f tmp_xphaselist) rm tmp_xphaselist
  if (-f tmp_yphaselist) rm tmp_yphaselist
  ...
  foreach line (`awk '{print $0}' $1`)
    ...
    if (-f $pth"xphase.grd") then
      echo $pth"tmp.PRM:"$pth"xphase.grd" >> tmp_xphaselist
    endif

    if (-f $pth"yphase.grd") then
      echo $pth"tmp.PRM:"$pth"yphase.grd" >> tmp_yphaselist
    endif
  end 
  ...
  if ($n1 > 5) then
    ...
    if (-f tmp_xphaselist) then
      merge_swath tmp_xphaselist xphase.grd $n1 $n2 > merge_log_xphase
    endif

    if (-f tmp_yphaselist) then
      merge_swath tmp_yphaselist yphase.grd $n1 $n2 > merge_log_yphase
    endif
  else
    ...
    if (-f tmp_xphaselist) then
      merge_swath tmp_xphaselist xphase.grd > merge_log_xphase
    endif

    if (-f tmp_yphaselist) then
      merge_swath tmp_yphaselist yphase.grd > merge_log_yphase
    endif
  endif
