...
if ($skip_master != 2) then
  ...
  #
  # Determine which swaths successfully produced interferograms
  #
  set useF1 = 0
  set useF2 = 0
  set useF3 = 0

  if (-d ../F1/intf) set useF1 = 1
  if (-d ../F2/intf) set useF2 = 1
  if (-d ../F3/intf) set useF3 = 1

  #
  # Build merge filelist dynamically
  #
  if ($useF1) then
    set pth1 = `ls ../F1/intf/*/*PRM | awk NR==1'{print $1}' | awk -F"/" '{for (i=1;i<NF;i++) printf("%s/",$i)}'`
    set prm1m = `echo $f1m | awk '{ print "S1_"substr($1,16,8)"_"substr($1,25,6)"_F"substr($1,7,1)".PRM"}'`
